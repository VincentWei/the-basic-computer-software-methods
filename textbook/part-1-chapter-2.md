...
我们知道，桌面系统通常运行在英特尔的 x86 架构处理器上，而这种处理器按照低地址存放低位数据的原则保存多字节数据对象>。假定整数 0x1234 是一个 16 位整数，则在 Intel x86 平台上，处理器将按图 2-2 所示的方式存储这个整数。即低地址A中保>存 0x34 这个字节，而在高地址 A+1 中，保存 0x12 这个字节。这种字节序称为“小头（little-endian）”。见图 2-2。
